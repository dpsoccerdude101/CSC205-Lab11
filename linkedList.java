...
public class LinkedSet<T> {
    ...
    public void insert(T str)
    {
        ...
        if(!this.member(str)) {
            this.body.insertAt(str,this.body.length());
        }
    }
    ...
    public LinkedSet<T> union(LinkedSet<T> otherSet)
    {
        ...
        // Write code here
        this.body.resetCurrentElement();
        for(int i = 0; i < this.body.length(); i++){
            resultBody.insertAt(this.body.nextElement(), i);
        }
        // Now insert all the elements of the other set that
        // are not in the set called result
        // Write code here
        int i = 0;
        otherBody.resetCurrentElement();
        while(otherBody.hasMoreElements()) {
            T t = otherBody.nextElement();
            if (!resultBody.member(t)) {
                resultBody.insertAt(t, i);
                i++;
            }
        }
    ...
    public LinkedSet<T> intersection(LinkedSet<T> otherSet)
    {
        ...
        int i = 0;
        this.body.resetCurrentElement();
        while(this.body.hasMoreElements()) {
            T t = this.body.nextElement();
            if (otherBody.member(t)) {
                resultBody.insertAt(t, i);
                i++;
            }
        }
